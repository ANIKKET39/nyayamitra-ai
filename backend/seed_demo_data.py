"""
Seed script — populates the SQLite database with realistic demo data so the
Dashboard and other pages look populated immediately for hackathon judging,
without requiring live use first.
...
print("Seeding demo data for NyayaMitra AI...")
...
for s in rti_samples:
    ...
    db.commit()
    print(f"  Created RTI application: {content['subject'][:60]}...")
...
for c in complaint_samples:
    ...
    record.pdf_path = pdf_path
    db.commit()
    print(f"  Created complaint letter: {content['subject'][:60]}...")

# --- Demo eligibility checks ---
elig_samples = [
...
db.commit()
print(f"  Created {len(elig_samples)} eligibility check records.")
...
db.commit()
print("  Created 1 demo chat session.")

db.close()
print("\nDemo data seeded successfully! Start the backend and visit the Dashboard to see it.")
